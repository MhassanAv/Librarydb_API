...
class adminStore {
    ...
    async authenticate(adminname, password) {
        ...
        const adminInfo = await conn.query('SELECT * FROM admins WHERE adminname=($1)', [adminname]);
        conn.release();
    ...
    async create(a) {
        try {
            // @ts-ignore
            const conn = await db_1.default.connect();
            const sql = 'INSERT INTO admins (adminname, fullname, password) VALUES($1, $2, $3) RETURNING *;';
            const result = await conn.query(sql, [
                a.adminname,
                a.fullname,
            ...
            throw new Error(`Could not add new admin or adminname is already in use ${a.adminname}. Error: ${err}`);
        }
    }
    async edit(a, id) {
        try {
            // @ts-ignore
            const conn = await db_1.default.connect();
            const sql = 'UPDATE admins SET adminname=$1, fullname=$2, password=$3 WHERE id=($4) RETURNING *;';
            const result = await conn.query(sql, [
                a.adminname,
                a.fullname,
                (0, auth_1.hash)(a.password),
